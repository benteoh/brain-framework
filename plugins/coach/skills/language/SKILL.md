...
- Compose the exercise surface per [references/exercise-surface-contract.md](references/exercise-surface-contract.md), using the `studio` skill's local runtime (`serve.mjs`/`open.mjs`/`say.mjs`) rather than any cloud artifact tool — the conversation happens live, in the page.
