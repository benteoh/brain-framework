...
- **Applies to drip sessions only** (a live back-and-forth loop: watching a transcript for replies and answering via `say.mjs`). Suggest the learner run `/effort low` for that kind of session — most of the perceived reply latency is agent turn/inference time, not the transport, and lower effort cuts it noticeably. Mention it once, at the start, since effort changes mid-conversation invalidate prompt caching. **Do not suggest it for a chapter session**: `chapter-shell` has no agent on the critical path, so there is nothing to speed up, and the two authoring turns it does have are exactly where effort should be highest.
...
- `scripts/serve.mjs --html <path> --out <transcript-path> [--data <path>] [--assets <dir>] [--port N]` — serves one HTML artifact plus a live, shared conversation transcript for it, and optionally a JSON data/state contract for a shell template (see below):
  ...
  - `GET /assets/<file>` (only if `--assets` was passed) serves static files from that directory, path-traversal guarded. It exists so a shell can load a large binary — currently the Stockfish WASM engine — without inlining megabytes of base64 into the template. Passing `--assets` also makes the server send `Cross-Origin-Opener-Policy: same-origin` and `Cross-Origin-Embedder-Policy: require-corp`, which is what browsers require before exposing `SharedArrayBuffer` and therefore WASM threads. Shells that ship no engine are unaffected, since the headers are only sent when `--assets` is in play.
...
As of this writing, the agent's per-session job is: **pick a checked-in shell template, write a small JSON data file, start `serve.mjs` pointed at both.** No markup generation. Five canonical shells live in `templates/`, each a fully self-contained HTML file (inline CSS/JS, no CDN, theme-aware light/dark CSS) that fetches its content from `GET /data` on load, hydrates chat history from `GET /transcript`, subscribes to `GET /events` (dispatching on `kind`), and has a conversation panel (transcript + input) posting to `POST /submit`:
...
- **`chapter-shell.html`** — the long-form shell, and the default for a teaching session (see the core `learn` skill's chapter mode). Data: `{title, subtitle?, estimatedMinutes, provenance, sections: [{id, heading, blocks: []}]}`, where each block is one of `prose`, `callout`, `board`, `game`, `exercise`, `compare`, `quiz`, `recap`. A whole 15–60 minute lesson in one scrollable document with a sticky section rail, rather than one card at a time.
  - **Feedback is pre-authored and revealed client-side**, so the learner never waits on an agent turn mid-lesson. The agent's job moves entirely into composing the payload.
  - **Every chessboard is engine-backed** when `--assets` points at an installed Stockfish (see the `coach:chess` skill's `setup-engine.mjs`). The shell picks the multithreaded build when the page is cross-origin isolated and the single-threaded build otherwise; if neither loads, all eval UI hides itself and every other feature keeps working.
  - Three pedagogical rules are enforced by the shell, not left to the author. **The answer stays hidden while the exercise is still live** — the eval bar, the engine's best move and the authored explanation are all revealed together, and only when the exercise closes, so that a throwaway wrong move is not the cheapest route to the solution. An unanticipated answer still gets an engine reply, but as the *gap* it costs rather than the move that closes it. **Every eval readout displays its search depth**, so a shallow flash eval cannot masquerade as a deep one. **Every exercise has an exit that is not guessing** — a `Show me the answer` control that closes it and emits `exercise-revealed`, because an exercise a learner cannot solve and cannot leave is worse than one they get wrong.
  - **Learner actions are emitted as structured events** to the transcript — `POST /submit` with a JSON-encoded `{kind: 'chapter-event', event, section, block, payload}`. Debrief from these rather than asking the learner how it went. Free-text questions from the coach panel arrive as plain text, so the two are trivially distinguishable. The exercise events are `exercise-attempted` (carrying `matched` and `correct`, which separate an *anticipated* wrong answer from one the author never considered), `exercise-solved`, `exercise-revealed`, `hint-revealed` and `ask-opened`. Solved unaided, solved after three hints, and given up on are three different results; a debrief that reports them as one number is not diagnostic.
  - `scripts/render-chapter-md.mjs --data <chapter.json>` renders the identical payload to Markdown with unicode diagrams and collapsed answers. This is the required fallback, not a nicety.
