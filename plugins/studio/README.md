...
To see a chapter without composing one, `node scripts/preview-chapter.mjs` serves the checked-in example payload in `chapter-shell`, verifies that the shell, the data and (with `--assets`) both engine builds actually return 200, and prints the click-through that checks the pedagogical rules still hold — a wrong answer must not reveal the eval, an unanticipated answer must not name the best move, and every exercise must have an exit that is not guessing.
